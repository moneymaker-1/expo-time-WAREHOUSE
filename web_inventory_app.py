...
def initialize_db():
    ...
    cursor = conn.cursor()
    # تحديث جدول الأصناف لدعم الوحدات والموردين
    ...
    # تحديث جدول العمليات لدعم أرقام السندات
...
# دالة توليد كود SKU تلقائياً لضمان التسلسل (مخفية)
...
def main():
    ...
    if low_stock_data:
        st.sidebar.error(f"⚠️ تنبيه: يوجد {len(low_stock_data)} أصناف شارفت على الانتهاء!")
    ...
    if choice == "🔍 عرض المخزون والإدارة":
        ...
        if data:
            ...
            if st.session_state.role == "مدير":
                st.write("🔧 **لوحة تحكم المدير المطلقة (تحديث سريع)**")
                target = st.selectbox("اختر الكود SKU للتعديل", [""] + [d[1] for d in data])
                if target:
                    ...
                    if st.button("✅ تنفيذ التعديل الآن"):
                    ...
                    if st.button("❌ حذف المنتج نهائياً من النظام"):
                        execute_query("DELETE FROM items WHERE sku=?", (target,))
                        st.error("تم حذف المنتج نهائياً"); st.rerun()

    # --- 2. إضافة صنف جديد (ترقيم تلقائي مخفي) ---
    elif choice == "➕ إضافة صنف جديد":
        st.subheader("إدخال صنف جديد إلى المخزن")
        with st.form("add_form"):
            name = st.text_input("اسم المنتج الجديد")
            col1, col2 = st.columns(2)
            qty = col1.number_input("الكمية الافتتاحية", min_value=0, step=1)
            unit = col2.selectbox("وحدة القياس", ["قطعة", "بكت", "جرام", "درزن", "كيلو"])
            price = st.number_input("القيمة (السعر)", min_value=0.0)
            supplier = st.text_input("اسم المورد")
            min_s = st.number_input("حد التنبيه للنواقص", value=5)
            ...
            if st.form_submit_button("حفظ المنتج"):
                if name:
                    ...
                    if execute_query("INSERT INTO items (name, sku, quantity, unit, price, supplier, min_stock, last_updated) VALUES (?,?,?,?,?,?,?,?)", 
                        ...
                        st.success(f"✅ تم الحفظ. الكود المولد: {new_sku}"); st.rerun()

    # --- 3. إدارة المستخدمين (حصري للمدير) ---
    elif choice == "👥 إدارة المستخدمين":
        st.subheader("إدارة طاقم العمل (الموظفين)")
        users_list, _ = fetch_query("SELECT username, role FROM users")
        st.table(pd.DataFrame(users_list, columns=['المستخدم', 'الدور']))
        
        with st.form("user_mgmt"):
            st.write("➕ إضافة موظف جديد")
            nu, np = st.text_input("اسم المستخدم"), st.text_input("كلمة المرور", type="password")
        ...
        st.markdown("---")
        u_del = st.selectbox("حذف موظف", [""] + [u[0] for u in users_list if u[0] != 'admin'])
        if st.button("❌ حذف الحساب المختار") and u_del:
            execute_query("DELETE FROM users WHERE username=?", (u_del,))
            st.success("تم الحذف"); st.rerun()

    # --- 4. تعريف منتج BOM (تعدد المكونات) ---
        ...
        if parent:
            with st.form("bom_form"):
                st.info("يمكنك ربط حتى 7 مكونات بالمنتج الواحد")
                rows = []
                for i in range(7):
                ...
                if st.form_submit_button("حفظ تركيبة BOM"):
                    ...
                    st.success("تم حفظ المكونات بنجاح")

    # --- 5. صرف أصناف مجمع (حتى 40 منتج + PDF) ---
    elif choice == "📤 صرف أصناف مجمع":
        if 'iss_rows' not in st.session_state: st.session_state.iss_rows = 1
        if st.button("➕ إضافة سطر جديد") and st.session_state.iss_rows < 40:
        ...
        with st.form("issue_form"):
            ...
            if st.form_submit_button("🚀 تنفيذ الصرف الجماعي وتوليد السند"):
                now = datetime.now()
                ...
                st.success(f"تم الصرف بالسند رقم: {do_ref}"); st.session_state.iss_rows = 1; st.rerun()
    ...
    # --- 6. طلب شراء PDF ---
        ...
        with st.form("po_form"):
            for i in range(st.session_state.po_rows):
                ...
                d = c3.date_input(f"التوريد المتوقع {i+1}", key=f"pd_{i}")
                if s: po_list.append((s.split("(")[1].split(")")[0], q, d.strftime("%Y-%m-%d")))
            ...
            if st.form_submit_button("📄 توليد وحفظ PDF"):
                ...
                st.download_button("📥 تحميل ملف PDF المولد", pdf_bytes, f"PO_{now_dt.strftime('%m%d')}.pdf", "application/pdf")

    # --- 7. سجل العمليات ---
    elif choice == "📜 سجل العمليات":
        st.subheader("سجل الرقابة وحركات المخزن")
