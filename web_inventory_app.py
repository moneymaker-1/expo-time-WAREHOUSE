...
def initialize_db():
    ...
    cursor = conn.cursor()
    # تحديث جداول النظام مع دعم الترقيم التلقائي ووحدات القياس
...
def main():
    ...
    if choice == "🔍 عرض وحذف الأصناف":
        ...
        if data:
            ...
            # تحويل الكميات لأرقام صحيحة عند العرض
            df['الكمية'] = df['الكمية'].astype(int)
            df['الحد الأدنى'] = df['الحد الأدنى'].astype(int)
            ...
            if st.session_state.role == "مدير":
                st.markdown("---")
                st.warning("⚠️ منطقة حذف الأصناف (للمدير فقط)")
    ...
    # --- 2. إضافة وتحديث (توليد كود تلقائي + وحدات) ---
        ...
        with st.form("item_form"):
            ...
            qty = st.number_input("الكمية", min_value=0, step=1, format="%d")
    ...
    # --- 3. صرف أصناف مجمع (DO) ---
        ...
        q_sel = col2.number_input("الكمية", min_value=1, step=1, format="%d")
    ...
    # --- 4. صرف وتجميع BOM ---
        ...
        st.subheader("تجميع المنتج وصرف المكونات")
        p_target = st.selectbox("المنتج النهائي", all_names)
        p_qty = st.number_input("كمية التجميع", min_value=1, step=1, format="%d")
        if st.button("🚀 تنفيذ التجميع"):
            ...
            if comps:
                ...
                st.success("تم صرف المكونات"); st.rerun()
    ...
    # --- 5. تعريف BOM ---
        ...
        with st.form("bom_reg"):
            ...
            req_qty = st.number_input("الكمية لكل وحدة", min_value=1, step=1, format="%d")
    ...
    # --- 6. طلب شراء وسجل العمليات ---
        ...
        for i in range(st.session_state.po_rows):
            ...
            q = c2.number_input(f"الكمية{i+1}", key=f"po_q_{i}", format="%d")
            d = c3.date_input(f"التوريد {i+1}", key=f"po_d_{i}")
